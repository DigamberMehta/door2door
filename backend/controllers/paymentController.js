...
export const refundPayment = async (req, res) => {
  try {
    ...
    if (refundAmount > payment.getRemainingAmount()) {
      return res.status(400).json({
        success: false,
        message: "Refund amount exceeds remaining amount",
      });
    }

    // Create refund with Yoco
    const refundData = {
      amountInCents: yocoService.formatAmount(refundAmount),
      reason: reason || "Customer requested refund",
    };

    const yocoResponse = await yocoService.createRefund(
      payment.yocoChargeId,
      refundData,
    );
    ...
    const refund = {
      refundId: yocoResponse.data.id,
      amount: refundAmount,
      reason,
      status: "succeeded",
      yocoRefundId: yocoResponse.data.id,
      processedAt: new Date(),
    };

    await payment.addRefund(refund);
